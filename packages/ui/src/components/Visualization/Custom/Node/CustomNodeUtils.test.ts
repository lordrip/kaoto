...
describe('CustomNodeUtils', () => {
  ...
  describe('getNodeDragAndDropDirection', () => {
    it('should return forward based on the path', () => {
      (vizNode1.getId as vi.Mock).mockReturnValueOnce('route1');
      (vizNode2.getId as vi.Mock).mockReturnValueOnce('route1');
    ...
    it('should return forward based on the entity id', () => {
      (vizNode1.getId as vi.Mock).mockReturnValueOnce('route1');
      (vizNode2.getId as vi.Mock).mockReturnValueOnce('route2');
      const result = getNodeDragAndDropDirection(vizNode2, vizNode1, false);
      expect(result).toBe('forward');
    });
    ...
    it('should return forward when nodes are at the same level', () => {
      ...
      (whenLogNode.getId as vi.Mock).mockReturnValueOnce('route');
      (otherwisePlaceholderNode.getId as vi.Mock).mockReturnValueOnce('route');
    ...
    it('should return backward when a container sub-node is dragged backward and dropped on the container connected edge', () => {
      ...
      (whenLogNode.getId as vi.Mock).mockReturnValueOnce('route');
      (choiceNode.getId as vi.Mock).mockReturnValueOnce('route');
  ...
  describe('handleValidNodeDrop', () => {
    const noopGetOnCopyAddons = vi.fn().mockReturnValue([]);

    it('should paste the dragged node as AppendStep when direction is forward', () => {
      handleValidNodeDrop(vizNode1, vizNode2, false, vi.fn(), noopGetOnCopyAddons);
    ...
    it('should paste the dragged node as PrependStep when direction is backward', () => {
      handleValidNodeDrop(vizNode2, vizNode1, false, vi.fn(), noopGetOnCopyAddons);
    ...
    it('should paste the dragged node as AppendStep when direction is forward, call the removeflow() instead', () => {
      ...
      (interceptVizNode.getNodeInteraction as vi.Mock).mockReturnValue({
        canRemoveStep: false,
        canRemoveFlow: true,
      });
      const removeFlowMock = vi.fn();
    ...
    it('should not paste the dragged node if the dragged node getCopiedContent() returns undefined', () => {
      (vizNode1.getCopiedContent as vi.Mock).mockReturnValueOnce(undefined);
      handleValidNodeDrop(vizNode1, vizNode2, false, vi.fn(), noopGetOnCopyAddons);
    ...
    it('should process copied content through onCopyAddon when provided', () => {
      ...
      (datamapperVizNode.getCopiedContent as vi.Mock).mockReturnValue(originalContent);

      const mockOnCopyAddon: IOnCopyAddon = {
        type: IInteractionType.ON_COPY,
        activationFn: vi.fn(),
        callback: vi.fn().mockReturnValue(transformedContent),
      };

      const getOnCopyAddons = vi.fn().mockReturnValue([mockOnCopyAddon]);

      handleValidNodeDrop(datamapperVizNode, vizNode2, false, vi.fn(), getOnCopyAddons);
    ...
    it('should not paste if onCopyAddon returns undefined', () => {
      const mockOnCopyAddon: IOnCopyAddon = {
        type: IInteractionType.ON_COPY,
        activationFn: vi.fn(),
        callback: vi.fn().mockReturnValue(undefined),
      };

      const getOnCopyAddons = vi.fn().mockReturnValue([mockOnCopyAddon]);

      handleValidNodeDrop(vizNode1, vizNode2, false, vi.fn(), getOnCopyAddons);
  ...
  describe('checkNodeDropCompatibility', () => {
    ...
    it('should return false if dropped node content is undefined', () => {
      (vizNode2.getCopiedContent as vi.Mock).mockReturnValueOnce(undefined);
      const result = checkNodeDropCompatibility(vizNode1, vizNode2, vi.fn());
      expect(result).toBe(false);
    });
